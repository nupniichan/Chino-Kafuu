"""
Mid-term memory: SQLite-based session storage with summarized conversations.
...
class MidTermMemory:
    ...
    def _init_database(self):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.info(f"Mid-term memory database initialized: {self.db_path}")
    ...
    def get_high_importance_summaries(
        ...
        """Retrieve high-importance summaries for long-term storage."""
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
